        ...
        const loadStats = async () => {
            try {
                ...
                if (db) {
                    ...
                    const occupied = await db.select<any[]>("SELECT COUNT(*) as count FROM bookings WHERE status = 'Checked-In' AND pension_id = ? AND (is_deleted = 0 OR is_deleted IS NULL)", [pensionId]);
